...
class SegmentUtils():
    ...
    def set_default_img( self, path_to_uploaded_img : str, frame_dim : tuple, threshold = 0.7 ):
        ...
        self._DEF_IMAGE = masked_image

    def is_human_present(self, condition):
        return np.sum(np.invert(condition)) > 0

    def display_default_img(self, frame, threshold=0.7):
